...
def freq_sampling_Hilbert(k):
  ...
  transition band is set on points 1, k, k+1, 2k
  '''
  N = 2 * k + 1 # length

  # set error on transition points
  # larger error on transition points leads to smaller ripple on the freq response
  trans_err = 0.1

  # set sampled frequency response
